...
def uniform_bin_func(x, n_bins):
    return jnp.linspace(jnp.min(x), jnp.max(x), n_bins + 1)
...
class KBinsDiscretizer:
    ...
    def fit(self, X, sample_weight=None, vectorize=True, *, remove_bin=False, remove_ref=1e-3):
        """Fit the estimator.
        ...
        if self.diverse_n_bins is None:
            if self.strategy == "uniform":
                ...
                bin_edges = jax.vmap(uniform_bin_func, in_axes=(1, None), out_axes=1)(X, n_bins)
            ...
            if self.strategy == "uniform":
                for index_n_bin in range(diverse_n_bins.shape[0]):
                    ...
                    if diverse_n_bin < n_bins:
                        maxval = jnp.max(x)
                        diverse_bin_edges = jnp.linspace(jnp.min(x), jnp.max(x), diverse_n_bin + 1)
                        if index_n_bin == 0:
                            bin_edges = jnp.concatenate([diverse_bin_edges, jnp.full((n_bins - diverse_n_bin), maxval)]).reshape(-1, 1)
                        else:
                            bin_edges = jnp.concatenate([bin_edges, jnp.concatenate([diverse_bin_edges, jnp.full((n_bins - diverse_n_bin), maxval)]).reshape(-1, 1)], axis=1)
                    else:
                        diverse_bin_edges = jnp.linspace(jnp.min(x), jnp.max(x), diverse_n_bin + 1).reshape(-1, 1)
                        if index_n_bin == 0:
                            bin_edges = diverse_bin_edges
                        else:
                            bin_edges = jnp.concatenate([bin_edges, diverse_bin_edges], axis=1)
            if self.strategy == "quantile":
                if sample_weight is None:
                    quantiles = jnp.linspace(0, 100, n_bins + 1)
                    for index_n_bin in range(diverse_n_bins.shape[0]):
                    ...
                    for index_n_bin in range(diverse_n_bins.shape[0]):
                        ...
                        diverse_bin_edges =  jax.vmap(_weighted_percentile, (None, 0, None))(
            ...
            if self.strategy == "kmeans":
                ...
                for index_n_bin in range(diverse_n_bins.shape[0]):
                    ...
                    minval = jnp.min(x)
                    maxval = jnp.max(x)
                    uniform_edges = jnp.linspace(minval, maxval, diverse_n_bin + 1)
                    init = (uniform_edges[1:] + uniform_edges[:-1])[:, None] * 0.5
                    km = KMEANS(
                        n_clusters=diverse_n_bin, n_samples=x.shape[0], init=init, max_iter=10
                    )
                    km.fit(x)
                    centers = jnp.sort(km._centers[:, 0])
                    diverse_bin_edges = jnp.r_[minval, (centers[1:] + centers[:-1]) * 0.5, maxval]
                    if index_n_bin == 0:
                        bin_edges = jnp.concatenate([diverse_bin_edges, jnp.full((n_bins - diverse_n_bin), maxval)]).reshape(-1, 1)
                    else:
                        bin_edges = jnp.concatenate([bin_edges, jnp.concatenate([diverse_bin_edges, jnp.full((n_bins - diverse_n_bin), maxval)]).reshape(-1, 1)], axis=1)
    ...
    def fit_transform(
        self, X, sample_weight=None, vectorize=True, *, remove_bin=False, remove_ref=1e-3
    ):
        """fit and transform with same input data.
        ...
        """
        return self.fit(
            X, sample_weight=sample_weight, vectorize=vectorize, remove_bin=remove_bin, remove_ref=remove_ref
        ).transform(X)
